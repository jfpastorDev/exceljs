  ...
  prepare(model, options) {
    ...
    if (model.charts) {
      model.charts.forEach(chart => {
        ...
        const chartId = nextRid(drawing.rels);
        chart.graphic.graphic.graphicData.chart.rId = chartId;

        const anchor = {
          graphicFrame: chart.graphic,
          range: chart.range,
          cellType: chart.cellType,
        };
        drawing.anchors.push(anchor);
        drawing.rels.push({
          Id: chartId, // chart.chartId,
          Type: 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/chart',
          Target: `../charts/${chart.name}.xml`,
        });
      });
    }
  ...
  reconcile(model, options) {
    ...
    if (model.drawing) {
      ...
      if (match) {
        ...
        const drawingSelected = options.drawings.find(drawing => Object.keys(drawing)[0] === drawingName);
        if (drawingSelected[drawingName]) {
          drawingSelected[drawingName].anchors.forEach(anchor => {
            if (anchor.picture) {
              const image = {
                type: 'image',
                ...
                cellType: anchor.cellType,
                name: anchor.picture.name,
              };
              model.media.push(image);
            } else if (anchor.graphicFrame) {
              // Anchor de chart
              const chart = {
                type: 'chart',
                chartId: anchor.graphicFrame.graphic.graphicData.chart.rId,
                cellType: anchor.cellType,
                graphic: anchor.graphicFrame,
                name: anchor.medium.chartName,
                range: anchor.range,
              };
